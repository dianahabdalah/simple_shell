...
/**
 * split_string - Breaks a string into words, ignoring repeat delimiters.
...
char **split_string(char *str, char *delimiter)
{
	...
	for (i = 0, j = 0; j < num_words; j++)
	{
		while (is_delimiter(str[i], delimiter))
			i++;
		k = 0;
		while (str[i + k] && is_delimiter(str[i + k], delimiter))
			k++;
...
/**
 * split_string_v2 - Splits string into words based on a specified delimiter.
 * @str: The input string
